...
function [tp8, powcons8] = cluster8(X,disratee,num_ue)
    disrate=disratee;
    figure;
    alpha=0.5;
    beta=0.5;
    ...
    plot(X(clusterX==8,1),X(clusterX==8,2),'ko','MarkerSize',15);
    drawnow;
    xlabel('X Coordinate');
    ylabel('Y Coordinate');
    ...
    for i = 1:num_ue
        ...
        elseif (clusterX(i)==7)
            ...
            M = Wavelength / (4 * pi * d(i));
            Pr0=PTxdBm + TXAntennaGain + RXAntennaGain- (20*log10(1/M));
            Pr8=Pr0+(10*PathLossExponent* log10(d(i)/Dref))+GaussRandom;
            pr8w = [pr8w 10^((-1*Pr8-30)/10)];
            randn('state', rstate); 
            X(i,3) = disrate(1,i);
            X(i,4) = Pr8;
            X(i,5)=alpha*X(i,3)+beta*X(i,4);
    ...
    pc8h = numel(ceight)*PTx - sum(pr8w);
    hold on;
    e1=[];
    e2=[];
    e3=[];
   ...
   for i=1:numel(e8)
       for j=1:numel(ceight(:,1))
           if(j~=e8(i))
               diss= sqrt((ceight(e8(i),1)-ceight(j,1))^2-(ceight(e8(i),2)-ceight(j,2))^2);
               M = Wavelength / (4 * pi * diss);
               Pr0=X(e8(i),4) + TXAntennaGain + RXAntennaGain- (20*log10(1/M));
               PrX=Pr0+(10*PathLossExponent* log10(ceight(i,6)/Dref))+GaussRandom;
               s8=sinnr(e8(i),ceight(e8(i),1),ceight(e8(i),2),ceight(j,1),ceight(j,2),ceight,PrX);
               ss8=[ss8 s8];
               se8=speceff(PrX,ceight(e8(i),4),ceight);
               sp8=[sp8 se8];
            else
                ss8=[ss8 0]; 
                sp8=[sp8 0];
   ...
   for j=1:numel(ceight(:,1))
       if(j~=rh)
           diss= sqrt((ceight(rh,1)-ceight(j,1))^2-(ceight(rh,2)-ceight(j,2))^2);
           M = Wavelength / (4 * pi * diss);
           Pr0=X(rh,4) + TXAntennaGain + RXAntennaGain- (20*log10(1/M));
           PrX=Pr0+(10*PathLossExponent* log10(ceight(i,6)/Dref))+GaussRandom;
           s8=sinnr(rh,ceight(rh,1),ceight(rh,2),ceight(j,1),ceight(j,2),ceight,PrX);
           sr8=[sr8 s8];
       else
           sr8=[sr8 0];
       end
    ...
    ss8=reshape(ss8,numel(e8),numel(a8));
    ...
    sp8=reshape(sp8,numel(e8),numel(a8));
    ...
    thwoa=[];
    thwoa=[throughput(rt1,100e7) throughput(rt2,100e7) throughput(rt3,100e7) throughput(rt4,100e7) throughput(rt5,100e7) throughput(rt6,100e7) throughput(rt7,100e7) throughput(rt8,100e7)];
    ...
    th4temp=[throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7)]; 
    ...
    plot(ceight(inde8,1),ceight(inde8,2),'kd','MarkerSize',15,'DisplayName','UEVBS8 with SINR')
    ...
    rp8 = recpow(inde8, ceight(inde8,1), ceight(inde8,2), ceight);
    ...
    pc8 = abs(pc8h + 10^((-1*ceight(inde8,4)-30)/10)*(numel(ceight)-1) - rp8);

    pctot = pc1+pc2+pc3+pc4+pc5+pc6+pc7+pc8;
    
    powcons8 = 10*log10(pctot*1000);
    
    figure;
    bar([1 2 3 4 5 6 7 8], [pc1 pc2 pc3 pc4 pc5 pc6 pc7 pc8], 0.4);
    ...
    figure;
    thh=[];
    thwa=[throughput(maxx1,100e7) throughput(maxx2,100e7) throughput(maxx3,100e7) throughput(maxx4,100e7) throughput(maxx5,100e7)  throughput(maxx6,100e7) throughput(maxx7,100e7) throughput(maxx8,100e7)];
    
    %Throughput of the algorithm selection
    thh=[th4temp;thwoa;thwa];
    barX = categorical({'Cluster 1','Cluster 2','Cluster3','Cluster 4','Cluster 5','Cluster 6','Cluster 7','Cluster 8'});
    barX= reordercats(barX,{'Cluster 1','Cluster 2','Cluster3','Cluster 4','Cluster 5','Cluster 6','Cluster 7','Cluster 8'});
    bar(barX,log10(transpose(thh)),0.4);
    ylabel('Sum rate in bits per second');
    legend({'LTE','Random Selection','Algorithm Selection'});
    title('Sum rate of the UE VBS network for 6 clusters');

    figure;
    bar([1 2 3 4 5 6 7 8],[q1 q2 q3 q4 q5 q6 q7 q8],0.4);
