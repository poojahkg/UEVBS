...
function [tp5, powcons5] = cluster5(X,disratee,num_ue)
    disrate=disratee;
    figure;
    alpha=0.5;
    beta=0.5;
   ...
   for j=1:numel(cfive(:,1))
    ...
    thwoa=[];
    thwoa=[throughput(rt1,100e7) throughput(rt2,100e7) throughput(rt3,100e7) throughput(rt4,100e7) throughput(rt5,100e7)];
    ...
    disp(ss4g);
    maxx4g=mean(ss4g);
    th4temp=[throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7) throughput(maxx4g,20e7)]; 
    ...
    pctot = pc1+pc2+pc3+pc4+pc5;
    
    powcons5 = 10*log10(pctot*1000);
    
    figure;
    bar([1 2 3 4 5], [pc1 pc2 pc3 pc4 pc5], 0.4);
    ylabel('Power consumed in each cluster (watts)');
    xlabel('Cluster number');
    title('Power consumption');

    figure;
    thh=[];
    thwa=[throughput(maxx1,100e7) throughput(maxx2,100e7) throughput(maxx3,100e7) throughput(maxx4,100e7) throughput(maxx5,100e7)];
    
    %Throughput of the algorithm selection
    thh=[th4temp;thwoa;thwa];
    barX = categorical({'Cluster 1','Cluster 2','Cluster3','Cluster 4','Cluster 5'});
    barX= reordercats(barX,{'Cluster 1','Cluster 2','Cluster3','Cluster 4','Cluster 5'});
    bar(barX,log10(transpose(thh)),0.4);
    ylabel('Sum rate in bits per second');
    legend({'LTE','Random Selection','Algorithm Selection'});
    title('Sum rate of the UE VBS network for 5 clusters');

    figure;
    bar([1 2 3 4 5],[q1 q2 q3 q4 q5],0.4);
